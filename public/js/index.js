...
ground.GetBody().SetUserData({id: "ground"})


var fixDefRoof = new b2FixtureDef;
fixDefRoof.density = 1.0;
fixDefRoof.friction = 0.5;
fixDefRoof.restitution = 0;
fixDefRoof.shape = new b2PolygonShape;
fixDefRoof.shape.SetAsBox((CANVAS_WIDTH/SCALE)/2, (100/SCALE)/2);
var bodyDefRoof = new b2BodyDef;
bodyDef.type = b2Body.b2_staticBody;
bodyDefRoof.position.x = (CANVAS_WIDTH/2/SCALE);
bodyDefRoof.position.y = (CANVAS_HEIGHT/SCALE) - (580/SCALE);
var roof = world.CreateBody(bodyDefRoof).CreateFixture(fixDefRoof);
roof.GetBody().SetUserData({id: "roof"})
...
// 4 Enemy's: 100 points deducted if pacman collides with the enemy
...
//Adding Pellets to the world, 100 points added everytime pacman collides with a pellet. 
...
//IF statements for the enemy positions (left, right, up or down)
if(enemyRight) {
...
//destroys the pellets from the world
...
//gameover function, outputs the final result in a dialog box and refreshes the game after the box is closed
gameOver();
...
function up() {
pacman.GetBody().ApplyImpulse(new b2Vec2(0, -1), pacman.GetBody().GetWorldCenter());
};
...
function stop() {
pacman.GetBody().SetLinearVelocity(new b2Vec2(0, pacman.GetBody().GetLinearVelocity().y));
};
...
//enemy collision, updates the boolean variable and results in the enemy changing direction. 
...
if(contact.GetFixtureA().GetBody().GetUserData().id == "enemy3" && contact.GetFixtureB().GetBody().GetUserData().id == "roof") {	
	enemyDown3 = true;	
	} else if (contact.GetFixtureB().GetBody().GetUserData().id == "enemy3" && contact.GetFixtureA().GetBody().GetUserData().id == "roof") {
	enemyDown3 = true;
};

if(contact.GetFixtureA().GetBody().GetUserData().id == "enemy3" && contact.GetFixtureB().GetBody().GetUserData().id == "ground") {	
	enemyDown3 = false;	
	} else if (contact.GetFixtureB().GetBody().GetUserData().id == "enemy3" && contact.GetFixtureA().GetBody().GetUserData().id == "ground") {
	enemyDown3 = false;
};

if(contact.GetFixtureA().GetBody().GetUserData().id == "enemy2" && contact.GetFixtureB().GetBody().GetUserData().id == "roof") {	
	enemyDown2 = true;	
	} else if (contact.GetFixtureB().GetBody().GetUserData().id == "enemy2" && contact.GetFixtureA().GetBody().GetUserData().id == "roof") {
	enemyDown2 = true;
};

if(contact.GetFixtureA().GetBody().GetUserData().id == "enemy2" && contact.GetFixtureB().GetBody().GetUserData().id == "ground") {	
	enemyDown2 = false;	
	} else if (contact.GetFixtureB().GetBody().GetUserData().id == "enemy2" && contact.GetFixtureA().GetBody().GetUserData().id == "ground") {
	enemyDown2 = false;
};

//Score Update occurs when collision with pacman and pellet occurs
...
//Adds the hit pellet to the array so it can be destroyed from the world. 
if(contact.GetFixtureA().GetBody().GetUserData().id == "pacman" && contact.GetFixtureB().GetBody().GetUserData().id == "pellet") {
